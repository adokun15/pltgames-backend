import { AddUserToDb, GetUserFromDb, UpdateUserFromDb } from "@/users/repositories/users.repository";
import { ChallongeAuthRequestHelper, ChallongeUserRequestHelper } from "@/utils/challonge/challongeRequestHelper"
...
export async function ExchangeCodeForToken(code: string) {
  ...
  const tokenUrl = `/oauth/token?code=${code}&client_id=${process.env.CHALLONGE_CLIENT_ID}&grant_type=authorization_code&redirect_uri=${process.env.CHALLONGE_CLIENT_REDIRECT_URI}`
    ...
    if(tokenRes.error){
      ...
      throw new Error(tokenRes.error_description)
    }

    // Decode token to get user profile; limit this often;
    const user_path = `/v2.1/me.json`; 
    ...
    //catch error from user endpoint;
    if(user?.error){
  ...
  const accessToken = jwt.sign(
   ...
   const data =  {
      user, accessToken, sessionId: session?.sessionId 
    };
...
export async function RefreshChallongeAccessTokenService(arg: { token: string}){
    try {
    const url = `
 ...
 return res;
    ...
    console.error("RefreshAccessTokenService Error:", e);
    throw e;
  }
}
...
export async function AuthorizeChallongeResourceService(arg: { token: string}){
  try {
    ...
    console.error("AuthorizeResourceService Error:", e);
